 ...
 */
async function sendOperationalAlertEmail(opts) {
  ...
  if (!recipient) {
    throw new Error('Destinatário obrigatório para e-mail de alerta operacional.');
  }
  const subj = String(subject || '').trim();
  if (!subj) {
    throw new Error('Assunto obrigatório para e-mail de alerta operacional.');
  }
  const bodyText = String(text || '').trim();
  if (!bodyText) {
    throw new Error('Corpo obrigatório para e-mail de alerta operacional.');
  }
